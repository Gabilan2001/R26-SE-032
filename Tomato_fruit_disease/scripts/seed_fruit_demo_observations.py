"""Seed DEMO FRUIT observation records into the existing SQLite schema.

Creates 100+ accepted FRUIT observations (Day 1 / Day 3 / Day 7 sequences)
using images from demo_data/observation_sequences_fruit/.

Marked as DEMO/SEEDED in weather_context. Does not retrain models.
Does not change gate/severity production logic.

  python scripts/seed_fruit_demo_observations.py
...
DEMO_PREFIX = "DEMO-FRUIT-"

# Planned affected-area % for variety (must obey 30% LOW/HIGH rule)
TREND_TEMPLATES = [
    # improving HIGH→HIGH→LOW
    ("improving_recover", [45.0, 32.0, 18.0], ["anthracnose", "anthracnose", "anthracnose"]),
    # worsening LOW→LOW→HIGH
    ("worsening", [15.0, 22.0, 38.0], ["blossom_end_rot", "blossom_end_rot", "blossom_end_rot"]),
    # stable LOW
    ("stable_low", [20.0, 21.0, 19.0], ["spotted_wilt_virus", "spotted_wilt_virus", "spotted_wilt_virus"]),
    # stable HIGH
    ("stable_high", [40.0, 42.0, 41.0], ["anthracnose", "anthracnose", "anthracnose"]),
    # recovered path
    ("recovering", [35.0, 28.0, 12.0], ["blossom_end_rot", "blossom_end_rot", "blossom_end_rot"]),
]
...
def _list_fruit_cases() -> list[Path]:
    if not FRUIT_KIT.is_dir():
        return []
    return sorted(
        [p for p in FRUIT_KIT.iterdir() if p.is_dir() and p.name.startswith("case_FRUIT_")]
    )


def _embedding_for(image_path: Path) -> list[float]:
    return predict_fruit_severity(image_path.read_bytes())["embedding"]


def seed(replace_demo: bool) -> dict:
    init_observation_db()
    cases = _list_fruit_cases()
    if len(cases) < 5:
        raise SystemExit(
            f"Need fruit demo kit images under {FRUIT_KIT}. "
            "Run: python scripts/build_fruit_observation_demo_kit.py --cases-per-class 25 --clean"
        )
    ...
    base_day = datetime(2026, 8, 1, 10, 0, 0, tzinfo=timezone.utc)
    day_offsets = [0, 3, 7]
    ...
    for case_dir in cases:
        ...
        # Mix template trends with bucket-aligned banks for variety
        if template_i % 3 == 0:
            bank = LOW_BANK if severity_bucket == "LOW" else HIGH_BANK
            pcts = [bank[i % len(bank)] for i in range(3)]
            # slight progression
        ...
        for idx, (day_off, fname, pct, disease) in enumerate(
            ...
            consistency = "BASELINE" if prev_emb is None else (
                "MATCH" if similarity is not None and similarity >= 0.85 else "POSSIBLE_MATCH"
            )
            trend = compute_trend(score, prev_score)
            trends.append(trend)
            status = compute_overall_status(trends)
            observation_id = f"OBS-{uuid.uuid4().hex[:10].upper()}"
            stored = save_observation_image(case["case_id"], observation_id, image_bytes)
            ...
            insert_observation(
                {
                    ...
                    "weather_context": {
                        "demo": True,
                        "seeded": True,
                        "demo_kind": "FRUIT_OBSERVATION_DEMO",
...
def main() -> int:
    ...
    parser.add_argument(
        "--replace-demo",
        action="store_true",
        help=f"Delete existing {DEMO_PREFIX}* cases before seeding",
    )
    args = parser.parse_args()
    summary = seed(replace_demo=args.replace_demo)
    print(json.dumps(summary, indent=2))
    if summary["observations_created"] < 100:
        print(
            f"WARNING: only {summary['observations_created']} observations "
            "(want >= 100). Increase fruit kit cases.",
            file=sys.stderr,
        )
        return 2
    return 0
